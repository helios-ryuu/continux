...
> Cập nhật ngày 22/05/2026: đã hoàn tất `docs/SETUP.md` từ §1 tới §9 trên cụm thật. Cụm K3s 3 server Ready, Argo CD/GitOps đã hoạt động, MinIO/Redpanda/RisingWave/VictoriaMetrics/Grafana Ready, dataset NYC TLC 2026-03 đã được convert sang JSONL và Vector đã chạy ingest có kiểm soát. Các secret runtime đã được che trong tài liệu.
...
## Hiện Trạng Sau Setup §1-9

| Hạng mục | Trạng thái | Bằng chứng hiện có | Việc còn lại |
|----------|------------|--------------------|--------------|
| Repo, docs, scripts, config | Đang cập nhật v0.2.1 | README/SETUP/TIMELINE/REPORT đồng bộ trạng thái triển khai | Chốt commit và tag/release nếu cần |
...
| Data plane | Hoàn tất nền tảng | MinIO deployed, Redpanda topic `nyc-taxi-events`, RisingWave pods `Running`, `SHOW CLUSTER` có 4 workers RUNNING | Sẵn sàng apply SQL §10 |
| Observability | Hoàn tất | VictoriaMetrics stack deployed, scrape configs Synced, Grafana rollout OK | Import/capture dashboard bằng chứng |
| Pipeline dữ liệu | Hoàn tất §9 | JSONL 450M, 3,952,451 rows, Vector `1/1 Running`, healthcheck passed | §10 apply SQL source/MV/sink |
| Báo cáo/kết quả | Đang thu thập | `k3s-check.sh` sau §9: Workloads Ready 100%, node-exporter 3/3 | Query output, Iceberg object, screenshot dashboard |
...
| 22/05 | Tài liệu v0.2.1 và SQL | Cập nhật output thực tế trong docs; apply SQL source/table/MV/sink; verify query | Đang làm |
| 23/05 | Iceberg và dashboard | Verify object output MinIO/Iceberg; import dashboard; chụp bằng chứng | Kế tiếp |
...
6. SQL chỉ apply sau khi secrets MinIO/RisingWave đúng. **Đang chuẩn bị §10.**
7. Báo cáo chỉ chốt sau khi có query output, object Iceberg và dashboard. **Còn lại.**
...
- [ ] README, SETUP, REPORT nhất quán với topology 3 máy và version `v0.2.1`.
...
| VPS thiếu RAM | Trung bình | Grafana/VictoriaMetrics restart | Grafana đã tăng resource ở v0.2.1; tiếp tục theo dõi dashboard |
