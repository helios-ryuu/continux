...
Sau khi tạo xong `key-risingwave`, tạo K8s Secret cho RisingWave state store S3/MinIO:
...
kubectl -n risingwave create secret generic risingwave-s3-credentials \
    --from-literal=AccessKeyID=key-risingwave \
    --from-literal=SecretAccessKey=<secret-key-cua-key-risingwave>
```
